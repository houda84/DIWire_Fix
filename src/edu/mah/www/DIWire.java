...
//  http://stackoverflow.com/questions/13370090/enums-in-processing-2-0
    ...
    class WirePoints  {
        ...
        int startPoint;
        WirePoints ()  {
            pts = new ArrayList<WirePoint>(); angles = new FloatList(); feeds = new FloatList();diameters = new FloatList();
            nAngleWarnings = nFeedWarnings = 0; modified = false; bendDir=false; curveIsClosed=false;
            startPoint=0;
        }
    }
    ...
    final float conversionFactor = 0.039370f;    // inches to mm
    ...
    final float xrev = (14.767f / .7f)/15;             // mm per rev
//final float xrev = 6.666666;             // mm per rev
    ...
    final float microSteps = 8; // microstepping for feed
...
//final float yrev = (119.7 / 1.125)/0.92363636363;   //added correction factor for 1/8" feed wheels
    final float yrev = 360.00f*( 16.00f / 20.00f);
    final float YG0Rate = 40;
    final float YAcceleration = 8000000;

    float fwDiam=48; // defaults to 1/8" value.  Updates later to actual value
...
//default constants are for 1/8" wire.  updated by selection of new material profile
    ...
    float wireDiam = 3.2258f; //mm (.127")   1.5875; //mm (0.0625in)
    float nAxisRad = kFactor*wireDiam; // radius to the neutral axis of the wire
    float mandrelRad = 3.1751f; //mm (.125")  1.5875; //(0.0625" in bend head mandrel radius)
    ...
    final float homeOffsetPos = 21.33f*xrev; //31.5; //30.7;
...
//final float pinMM2pinAng8 = 4.5;
    final float pinMM2pinAng = 6.6666666f/xrev;
